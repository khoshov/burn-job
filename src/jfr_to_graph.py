#!/usr/bin/env python3
"""
Profiler Call Graph Ingestor for KùzuDB Wrapper.
Forwards to core.graph_store.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Ingest JFR or Collapsed Stack Traces into KùzuDB")
    parser.add_argument("--input", "-i", required=True, help="Path to profile file")
    parser.add_argument("--db-path", "--db", default=os.path.join(REPO_ROOT, "profiler_graph.db"), help="KùzuDB directory")
    args = parser.parse_args()

    store = KuzuGraphStore(args.db_path)
    store.ingest_profile(args.input)
    print(f"✅ Successfully ingested '{args.input}' into KùzuDB graph at '{args.db_path}'.")

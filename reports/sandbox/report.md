...
5. **Мульти-вариантный Feature Toggle бенчмаркинг (`benchmark_variants.py` & `evaluate_variants_via_kuzu.py`):** Прогон всех альтернативных способов исправления из `FIX_VARIANTS.md`, сравнение метрик и выбор наилучшего решения.

---

### ⚡ Гибридная двухуровневая архитектура (Граф-БД + LLM Agent)

* **Уровень 1: Локальный C++/Python граф-движок KùzuDB (Выполнение < 50 мс)**
  - Парсинг стектрейсов, создание графа вызовов, выполнение 9 графовых анализаторов таксономии (T1–T9), проверка исключений Раздела 7 и ранжирование мульти-вариантных прогонов через Cypher исполняются локально в C++ памяти без вызовов внешних API.
  - Это обеспечивает мгновенное обнаружение узких мест и математически точный выбор наилучшего варианта решения.

* **Уровень 2: Генеративный слой LLM (DeepSeek / Antigravity)**
  - LLM-агент принимает от KùzuDB готовые агрегированные данные и блокированный вариант рефакторинга.
  - LLM генерирует чистый, высокопроизводительный Java-код (`*_FixedByAgent.java`), исключая затраты времени и токенов на разбор сырых логов.

---

### ⚡ Почему полный цикл аналитики и выбора решения занимает < 1 секунды

1. **Динамический Feature Toggle вместо пересборки (0 мс задержки):**
   Все варианты альтернативных исправлений (`v1`, `v2`, `v3`) скомпилированы в сервисы Spring Boot заранее и переключаются на лету через query param `?variant=v1|v2|v3`. Не требуется перезапуск JVM или пересборка байт-кода при тестировании гипотез.
2. **Локальный C++ инджестинг `.collapsed` в KùzuDB (12 мс):**
   `jfr_to_graph.py` конвертирует свёрнутые стектрейсы `caller;callee count` в графовую структуру узлов `Method` и ребер `CALLS` за 12 миллисекунд напрямую в векторной C++ памяти.
3. **Инкрементальная компиляция Java 21 (< 0.5 сек):**
   `javac` компилирует создаваемые агентом генеративные файлы `*_FixedByAgent.java` за 0.46 секунды без затрат времени на тяжелые фазы сборки.

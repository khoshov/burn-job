...
## 🛠️ Полная пошаговая инструкция для ручного запуска (Step-by-Step Manual Execution)

Вы можете последовательно выполнить каждую команду вручную для подготовки, сборки, анализа и генерации отчётов:

### 1️⃣ Настройка окружения и конфигурации
```bash
# 1.1. Создать локальный файл конфигурации из примера:
cp .env.example .env

# 1.2. Проверить переменные (.env настроен на тестовый проект ./test_project/src/main/java):
cat .env
```

### 2️⃣ Сборка и проверка тестового Java-проекта (`test_project`)
```bash
# 2.1. Перейти в тестовый проект и выполнить Maven-сборку:
cd test_project && mvn clean verify && cd ..
```

### 3️⃣ Установка зависимостей Python и окружения
```bash
# 3.1. Создание виртуального окружения и установка пакетов:
make setup

# 3.2. Или ручная инициализация:
python3 -m venv .venv
source .venv/bin/activate
pip install -e .
```

### 4️⃣ Запуск тестов фреймворка Burn Job
```bash
# 4.1. Прогон всех 31 unit и integration тестов:
make test
```

### 5️⃣ Пошаговое выполнение всех этапов оптимизации
```bash
# 5.1. Сканирование REST-контроллеров тестового проекта (19 эндпоинтов):
./run.sh scan

# 5.2. Импорт стека профилирования JFR/collapsed в граф-БД KuzuDB:
./run.sh ingest --profile ./app_profiling_full.collapsed --db ./profiler_graph.db

# 5.3. Запуск полного 8-шагового цикла в режиме отчёта (Read-Only, без изменения файлов):
./run.sh run-cycle

# 5.4. Запуск с автоматическим применением исправлений в код:
./run.sh run-cycle --apply

# 5.5. Запуск с локальной моделью Qwen3 через llama.cpp:
./run.sh run-cycle --backend llama.cpp --model-path Qwen3-4B/qwen3-4b-instruct.gguf
```

### 6️⃣ Просмотр сгенерированных отчётов
```bash
# 6.1. Открыть детальный Markdown отчёт Qwen3 AI с вариантами оптимизации:
cat reports/sandbox/detailed_report.md

# 6.2. Открыть структурированный JSON отчёт с аномалиями:
cat reports/sandbox/findings.json
```

---

## 🚀 Быстрый запуск одной командой (Quick Start)
...
# Запустите автономный цикл одной командой из корня проекта:
./run.sh
...
1. [Быстрый запуск (Quick Start)](#-быстрый-запуск-quick-start)

...
## 🚀 Быстрый старт с DeepSeek (рекомендованный способ)

**Что такое DeepSeek и зачем он нужен?**

DeepSeek — это внешняя LLM (языковая модель), которая генерирует варианты оптимизированного Java-кода для найденных дефектов. В отличие от локальной модели Qwen3 (которая требует скачивания GGUF-файла на ~8 GB), DeepSeek работает через облачное API — ничего скачивать не нужно, генерация кода происходит быстрее и качественнее. По умолчанию используется модель `deepseek-v4-flash`.
...
Скопируйте репозиторий и создайте виртуальное окружение Python:
...
# Создать виртуальное окружение Python (изолированная среда, чтобы зависимости не конфликтовали с системными пакетами)
python3 -m venv .venv

# Активировать окружение (теперь все pip-установки пойдут в .venv/)
source .venv/bin/activate

# Установить Burn Job и его зависимости в режиме editable (флаг -e — изменения в коде сразу подхватываются)
pip install -e .
```

**Зачем это нужно:** Burn Job написан на Python и требует библиотеки: `kuzu` (графовая БД для хранения профилей), `urllib` (HTTP-запросы к DeepSeek API), `rich` (цветной CLI-вывод). Виртуальное окружение изолирует эти зависимости от других Python-проектов на вашей машине.

---

### Шаг 2: Получение API-ключа DeepSeek

Burn Job использует DeepSeek API для генерации кода. Вам нужен ключ:

1. Зарегистрируйтесь на [platform.deepseek.com](https://platform.deepseek.com)
2. Пополните баланс (генерация кода для одного прогона стоит ~$0.01-0.05)
3. Создайте API-ключ в разделе "API Keys"
4. Скопируйте ключ (начинается с `sk-...`)

**Зачем нужен ключ:** DeepSeek — платное облачное API. Ключ аутентифицирует ваши запросы и списывает средства за каждый вызов модели. Без ключа генерация вариантов кода работать не будет.

---

### Шаг 3: Конфигурация `.env`

Скопируйте пример конфигурации и укажите ваш API-ключ DeepSeek:
...
Минимальная конфигурация для работы с DeepSeek выглядит так:

```env
# ----- Путь к вашему Java-проекту -----
# Укажите директорию, где лежат исходники Java (папка с package-директориями)
BURN_JOB_SRC_DIR=./test_project_light/src/main/java

# ----- Ключ DeepSeek API (обязательно) -----
DEEPSEEK_API_KEY=sk-ваш_ключ_здесь

# ----- Модель DeepSeek для генерации кода -----
DEEPSEEK_MODEL=deepseek-v4-flash

# ----- Остальное можно оставить по умолчанию -----
BURN_JOB_PROFILE_PATH=./app_profiling_full.collapsed
BURN_JOB_DB_PATH=./profiler_graph.db
```

**Проверьте, что ключ работает:**

```bash
# Быстрая проверка: отправить тестовый запрос к DeepSeek
curl -s https://api.deepseek.com/v1/models \
  -H "Authorization: Bearer $(grep DEEPSEEK_API_KEY .env | cut -d= -f2)" \
  | head -c 200
# Ожидаемый ответ: JSON со списком доступных моделей, включая deepseek-v4-flash
```

**Зачем нужен `.env`:** Все настройки вынесены в переменные окружения, чтобы не хардкодить пути и ключи в Python-коде. `.env` добавлен в `.gitignore` — ваш API-ключ случайно не попадёт в репозиторий.

---

### Шаг 4: Установка Java и Maven (если ещё не установлены)

Burn Job компилирует и запускает Java-приложение, поэтому нужны JDK и Maven:

```bash
# Проверить Java (нужна версия 17 или выше)
java -version
# Ожидаемый вывод: openjdk version "17.x.x" или выше

# Проверить Maven (нужна версия 3.8+)
mvn --version
# Ожидаемый вывод: Apache Maven 3.8.x

# Если Java не установлена — скачайте JDK 17+:
...
**Зачем это нужно:** Burn Job не просто анализирует исходники — он реально компилирует сгенерированные LLM варианты (`mvn compile`), запускает embedded Spring Boot и замеряет latency через Micrometer. Без JDK и Maven эта цепочка не работает.

---

### Шаг 5: Сборка целевого Java-проекта

Перед запуском анализа проект должен скомпилироваться без ошибок (baseline):

```bash
# Перейти в директорию вашего Java-проекта
cd ./test_project_light

# Собрать проект и прогнать тесты (clean — удалить старые артефакты, verify — полная верификация)
mvn clean verify -q

# Вернуться в корень Burn Job
cd ..
```

**Зачем это нужно:** Burn Job будет модифицировать файлы и перекомпилировать их. Если baseline не компилируется — бессмысленно что-то оптимизировать. Флаг `-q` (quiet) подавляет избыточный вывод Maven, оставляя только ошибки, если они есть.

**Ожидаемый результат:** Команда завершается без ошибок (пустой вывод или `BUILD SUCCESS`). В директории проекта появится папка `target/` со скомпилированными `.class`-файлами.
...
| Шаг | Действие | Зачем | Кто выполняет |
|-----|----------|-------|---------------|
| **1/8** | Сканирование `@RestController` в Java-исходниках | Найти все REST-эндпоинты для нагрузочного теста | Rule-based (парсинг AST) |
| **2/8** | Генерация Python-скрипта нагрузочного теста | Создать скрипт, который будет стрелять запросами в эндпоинты | Rule-based (шаблон) |
| **3/8** | Исполнение нагрузочного теста | Нагреть JIT-компилятор и собрать профиль CPU | subprocess + Python |
| **4/8** | Инжест профиля async-profiler в графовую БД KùzuDB | Построить граф вызовов для анализа hotspot-методов | Cypher MERGE |
| **5/8** | Детекция дефектов T1–T9 | Найти проблемы: N+1, Pattern.compile в цикле, Double boxing, и т.д. | Cypher + AST |
| | → **Генерация вариантов кода через DeepSeek** | 3 варианта исправления для каждого дефекта | **DeepSeek API** |
| | → **Бенчмарк каждого варианта** | Замерить latency через Micrometer (500 запросов на вариант) | Embedded Spring Boot |
| **6-7/8** | Оценка и выбор победителя | Выбрать вариант с наименьшим avg latency | Rule-based (по benchmark) |
| **8/8** | Финальная Maven-верификация | Убедиться, что проект всё ещё компилируется | `mvn test-compile` |

**Ожидаемое время выполнения:** ~2-5 минут, из которых:
- ~40-60 сек на два запроса к DeepSeek API (по ~30 сек на finding)
- ~30-60 сек на бенчмарк вариантов (запуск Spring Boot, 500 запросов на вариант, перезапуск)
- ~5 сек на всё остальное (сканирование, нагрузочный тест, инжест)

---

### Шаг 7: Просмотр результатов

После завершения цикла откройте сгенерированные отчёты:
...
Для каждого дефекта отчёт содержит:

```
### Finding #1: [T4] LightController.java (Line 17)
🤖 LLM: deepseek-v4-flash                          ← какая модель генерировала код

📋 Сравнение вариантов исправлений:
| Вариант | AST Score | Бенчмарк (avg) | Результат |
|---|---|---|---|
| Primitive Unboxing | 92.0 | ⏱ 0.000141s | 🏆 Победитель |
| StringBuilder Cap. | 100.0 | ⏱ 0.000148s | ⚪ Альтернатива |
| Array Pool | 100.0 | ⏱ 0.000146s | ⚪ Альтернатива |
```

- **AST Score** — аналитическая оценка сложности кода (0-100, на основе количества операций и вложенности)
- **Бенчмарк (avg)** — средняя latency 500 HTTP-запросов к эндпоинту с этим вариантом кода. **Чем меньше — тем быстрее**
- **Победитель** выбирается по benchmark, AST Score используется только как tiebreaker
- Если вариант не скомпилировался → в бенчмарке будет `❌ compilation_failed`

**Формат строки победителя в консоли:**
```
🏆 «Primitive Unboxing»: 0.000141s avg.  «Array Pool»: 0.000146s avg.  «StringBuilder»: 0.000148s avg. 
Победитель по benchmark: «Primitive Unboxing» (0.000141s).
```
...
# Импорт профиля async-profiler в KuzuDB (шаг 4 отдельно)
./run.sh ingest --profile ./app_profiling_full.collapsed --db ./profiler_graph.db

# Снять JFR-профиль с запущенного Java-приложения
./run.sh profile --duration 15 --output ./app_profiling.jfr

# Запуск с локальной моделью Qwen3 (не требует API-ключа, но нужен GGUF-файл ~8 GB)
./run.sh run-cycle --backend llama.cpp --model-path Qwen3-4B/qwen3-4b-instruct.gguf

# Запуск только variant generation через DeepSeek (+ локальная детекция через llama.cpp)
./run.sh run-cycle --backend llama.cpp --variant-llm deepseek --online
```
...
### 🐳 Запуск через Docker / Docker Compose (альтернативный способ)

Docker-образ включает всё необходимое, кроме API-ключа, который передаётся через переменную окружения:

```bash
# Через Docker Compose (автоматически собирает образ и запускает контейнер)
DEEPSEEK_API_KEY=sk-ваш_ключ docker compose up

# Вручную: сборка образа
docker build -t burn-job .

# Запуск контейнера с пробросом ключа и кода
docker run --rm \
  -e DEEPSEEK_API_KEY=sk-ваш_ключ \
  -v $(pwd):/app \
  burn-job run-cycle --backend openai --online
```
...
1. [Быстрый старт с DeepSeek](#-быстрый-старт-с-deepseek-рекомендованный-способ)
...
Все параметры работы пайплайна (путь к проверяемому коду, настройки БД, параметры LLM и профайлера) вынесены в `.env`.

Для первого запуска скопируйте пример файла конфигурации:

```bash
# Скопируйте пример конфигурации в локальный файл .env
cp .env.example .env
```

После этого при необходимости укажите в `.env` путь к вашему Java-проекту (`BURN_JOB_SRC_DIR`) или ключи API для LLM:
...
BURN_JOB_MODEL_PATH=./Qwen3-4B/qwen3-4b-instruct.gguf
```
...
| `DEEPSEEK_API_KEY` | — | API-ключ DeepSeek для генерации вариантов кода (обязателен для `--variant-llm deepseek`) |
| `DEEPSEEK_MODEL` | `deepseek-v4-flash` | Модель DeepSeek для генерации вариантов кода |

#!/usr/bin/env python3
"""
AST & Pattern Complexity Analyzer for Java Source Code (ported and adapted from SysLLMatic).

Analyzes Java source files for loop nesting depths (O(N), O(N^2), O(N^3)), container access patterns,
linear searches in loops, string concatenation in hot paths, and recursive recomputations.
"""
...
def _strip_comments(code: str) -> str:
    """Removes single-line and multi-line comments while respecting string literals."""
    in_string = False
    string_char = None
    i = 0
    result = []
...
def _detect_loop_nesting(code_stripped: str):
    """Tracks nested loops (for / while) and computes max nesting depth."""
    open_loops = []
    nesting_info = []
    max_nesting = 0
    i = 0
...
def analyze_complexity(source_code: str, language: str = 'java') -> dict:
    """Analyzes source code complexity and returns estimated complexity, issues, and suggestions."""
    try:
